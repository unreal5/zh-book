...
\subsection{设计思路}
利用 Gameplay Abilities 的事件监听机制：在敌人上激活一个能够监听特定事件的技能（\dfntxt{Ability}），当收到包含必要数据的事件载荷时，该技能解析并执行相应的行为。为了自动在赋予时激活某些技能，需要在 AbilitySystem 的创建/赋予流程中加入自定义逻辑，因此创建一个派生自 UAbilitySystemComponent 的自定义组件是合理且必要的。
